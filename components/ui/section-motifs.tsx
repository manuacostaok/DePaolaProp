...
function HouseIcon({ x, scale = 1 }: { x: number; scale?: number }) {
  return (
    <g transform={`translate(${x + 75 - 50 * scale}, ${80 - 80 * scale}) scale(${scale})`}>
      <path d="M10 90 V50 L50 15 L90 50 V90 Z" stroke="currentColor" strokeWidth="2" />
      <rect x="40" y="62" width="20" height="28" stroke="currentColor" strokeWidth="2" />
      <rect x="17" y="58" width="15" height="15" stroke="currentColor" strokeWidth="2" fill="currentColor" fillOpacity="0.18" />
      <rect x="68" y="58" width="15" height="15" stroke="currentColor" strokeWidth="2" />
    </g>
  );
}
...
function PersonIcon({ x, scale = 1 }: { x: number; scale?: number }) {
  return (
    <g transform={`translate(${x + 75 - 50 * scale}, ${90 - 90 * scale}) scale(${scale})`}>
      <circle cx="50" cy="28" r="17" stroke="currentColor" strokeWidth="2" />
      <path d="M14 94 Q14 54 50 54 Q86 54 86 94" stroke="currentColor" strokeWidth="2" />
    </g>
  );
}
...
function DocumentIcon({ x, scale = 1 }: { x: number; scale?: number }) {
  return (
    <g transform={`translate(${x + 75 - 35 * scale}, ${85 - 85 * scale}) scale(${scale})`}>
      <rect x="8" y="4" width="60" height="82" rx="2" stroke="currentColor" strokeWidth="2" />
      <line x1="20" y1="26" x2="56" y2="26" stroke="currentColor" strokeWidth="2" strokeOpacity="0.55" />
      <line x1="20" y1="40" x2="56" y2="40" stroke="currentColor" strokeWidth="2" strokeOpacity="0.55" />
      <line x1="20" y1="54" x2="42" y2="54" stroke="currentColor" strokeWidth="2" strokeOpacity="0.55" />
    </g>
  );
}
...
function MotifRow({ variant, count = 9, className }: { variant: keyof typeof ICONS; count?: number; className?: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} 150`} preserveAspectRatio="xMidYMax meet" fill="none" className={className} aria-hidden="true">
...
// Banda de fondo con deriva horizontal continua (loop CSS, ver
...
export function DriftBand({
  ...
  return (
    <div
      aria-hidden="true"
      className={cn("pointer-events-none absolute inset-x-0 bottom-0 overflow-hidden", heightClassName, className)}
      style={MASK_STYLE}
    >
      <div
