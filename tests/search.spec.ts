...
test("El buscador filtra por zona y operación", async ({ page }) => {
  await page.goto("/propiedades?zona=villa-martelli&operacion=venta");

  await expect(page.locator("select[name=zona]")).toHaveValue("villa-martelli");
  await expect(page.locator('input[name=operacion][value=venta]')).toBeChecked();

  // Villa Martelli + Venta da 3 resultados con el inventario real (ver seed.ts)
...
test("Filtro por característica: Terraza devuelve solo propiedades con esa feature", async ({ page }) => {
  await page.goto("/propiedades");

  // Inventario real: 2 propiedades tienen la feature "terraza"
  // ("casa-5-ambientes-grecia-villa-martelli" y "casa-3-ambientes-florida-oeste").
  await page.getByRole("checkbox", { name: "Terraza" }).check();
  await page.getByRole("button", { name: "Buscar" }).click();

  await expect(page.getByText(/^2 propiedades$/)).toBeVisible();
  await expect(page.locator('main a[href="/propiedades/casa-3-ambientes-florida-oeste"]').first()).toBeVisible();
});

test("Filtro por característica: Jardín agrupa variantes con y sin tilde de la misma key", async ({ page }) => {
  // El inventario real tiene "jardín" y "jardin" cargados como keys
  // distintas (inconsistencia de tipeo entre agentes) — el checkbox único
  // debe matchear las 3 propiedades reales que tienen cualquiera de las
  // dos, sin tocar los datos ni mostrar dos checkboxes duplicados.
  await page.goto("/propiedades");
...
test("Orden: precio menor a mayor arranca en USD y en el más barato", async ({ page }) => {
  ...
  // USD 99.900 es el precio más bajo del inventario real — los listados en
  // ARS quedan agrupados aparte para no comparar monedas distintas como si
  // fueran el mismo número.
  const firstCard = page.locator('main a[href^="/propiedades/"]').first();
  await expect(firstCard).toHaveAttribute("href", "/propiedades/departamento-2-ambientes-vergara-florida");
});
...
test("Orden: precio mayor a menor arranca en el USD más caro", async ({ page }) => {
  ...
  const firstCard = page.locator('main a[href^="/propiedades/"]').first();
  await expect(firstCard).toHaveAttribute("href", "/propiedades/chalet-6-ambientes-martinez");
});
...
test("Orden: mayor/menor superficie usa coveredArea (totalArea está vacío en la mayoría del inventario real)", async ({ page }) => {
  // chalet-6-ambientes-martinez tiene la mayor superficie cubierta real
  // (205m²), monoambiente-campus-norte-villa-martelli la menor (38m²) — se
  // compara la posición relativa entre ambas, no la posición absoluta #1,
  // porque la propiedad sin superficie cargada (coveredArea null) empata
  // y queda agrupada antes, sin orden determinístico contra las demás.
  await page.goto("/propiedades");
  ...
  const mayorIdx = desc.indexOf("/propiedades/chalet-6-ambientes-martinez");
  const menorIdx = desc.indexOf("/propiedades/monoambiente-campus-norte-villa-martelli");
  ...
  const mayorIdxAsc = asc.indexOf("/propiedades/chalet-6-ambientes-martinez");
  const menorIdxAsc = asc.indexOf("/propiedades/monoambiente-campus-norte-villa-martelli");
