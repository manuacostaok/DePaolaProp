...
**Verificado:** reproducido el error 2/2 veces en el deploy real antes del fix; suite completa de Playwright (21/21) en verde con el fix aplicado; verificación visual manual confirmando que el header transparente-sobre-hero se sigue viendo igual que antes. Falta confirmar en el deploy real una vez que este fix esté en producción (no se puede verificar la ausencia del error en Vercel hasta el próximo deploy).
