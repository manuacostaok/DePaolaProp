...
**Fases 1 a 5 completas, más la parte de Fase 6 que no depende de comprar un dominio.** Verificado en producción real y pusheado a `main`. Cada fase corrió su propia ronda de `tsc`/lint/build/Playwright antes de commitear; la suite completa terminó en 51/51 tests verdes.
...
- **Fase 4 (Analytics):** GA4 (`@next/third-parties`) wireado y 10 de los 12 eventos de la Fase 20 instrumentados de verdad (`share_property`/`ai_search` deliberadamente sin disparo — no hay feature todavía). Lighthouse real corrido contra producción (`PERFORMANCE-BASELINE.md`), que encontró y permitió corregir 3 bugs de accesibilidad reales no listados en la auditoría original (`select-name`, `heading-order`, `color-contrast` — este último con causa raíz no obvia: la opacidad que dimeriza las tarjetas de ejemplo arrastraba el badge y el label de barrio por debajo del contraste mínimo, matemáticamente confirmado; `heading-order` necesitó una segunda vuelta porque `/propiedades` no tiene ningún `h2` en toda la página, así que hasta un `h3` en el Footer seguía saltando un nivel). Detalle completo en la sección de Fase 4.
- **Fase 5 (Experiencia inmobiliaria):** filtro por característica y orden de resultados en el buscador; mapa real (no placeholder) en `/zonas/[zona]`, con una corrección real sobre el plan original (`Neighborhood` no tiene lat/lng propio, se deriva del promedio de sus propiedades); clustering navy en los 3 mapas del sitio; newsletter segmentado nuevo en Insights con un modelo Prisma aditivo (`NewsletterSubscriber`). Un bug real de infraestructura de tests encontrado y corregido (no de producto): `test.afterAll` corre una vez por worker bajo `fullyParallel: true`, no una vez al final — causaba que un worker borrara por error datos de prueba que otro worker necesitaba. Detalle completo en la sección de Fase 5.
- **Fase 6 (Producción) — parcial, a pedido explícito del usuario ("segui con la fase 6 lo que se pueda sin dominio"):** verificado que `SITE_URL` ya se usa consistentemente en sitemap/robots/metadataBase/JSON-LD (nada que corregir); 3 redirects reales desde URLs de Wix confirmadas en vivo (con un hallazgo real: `/proyectoresidencial` es la Revista Digital, no un emprendimiento, a pesar de lo que sugiere el nombre del path); corregido que los errores de `error.tsx`/`global-error.tsx` en realidad NUNCA llegaban a los logs de Vercel (corrían en el cliente) — ahora sí, vía una ruta server-side nueva sin dependencias; `RUNBOOK.md` con el proceso de restore de Neon, con lo que no se pudo confirmar (plan/retención actual) marcado explícitamente. Detalle completo en la sección de Fase 6.

**Se avanzó por instrucción explícita del usuario más allá del punto donde este agente se había detenido solo — primero hasta el final de la Fase 5, después con la parte de Fase 6 que no depende de comprar un dominio.** Lo que sigue bloqueado son decisiones y acciones que este agente no puede ni debe tomar unilateralmente aunque la instrucción sea "seguir con todo lo que se pueda": comprar el dominio en sí (Fase 6.1) es un gasto de dinero real y una acción de una sola vía; conectar Search Console (Fase 4.3) y GA4 con un ID real (Fase 4.1) necesitan acceso a la cuenta de Google de De Paola; confirmar el plan/retención de Neon (Fase 6.4) necesita acceso al dashboard de esa cuenta; elegir proveedor de CRM/automatización (Fase 7) y de IA (Fase 8) son decisiones de negocio y de costo recurrente, no técnicas. El resto del documento de abajo es el plan original sin cambios.
...
**FASE 3 — ✅ COMPLETA con una corrección importante encontrada durante el testing (2026-09-02).**

- **3.1 canonical:** agregado a las 22 páginas con metadata (estáticas y dinámicas) + fallback `canonical: "/"` en el layout raíz para Home (que no tenía metadata propio). Verificado con `fetch` real: `/propiedades?zona=martinez` → canonical sin querystring; ficha de propiedad → su propia URL. 5 tests nuevos en `tests/seo.spec.ts`.
- **3.2 loading.tsx/error.tsx — con una corrección real encontrada al testear:** `app/error.tsx` y `app/global-error.tsx` se implementaron sin problemas (verificado manualmente forzando un error real: se ve la página con marca completa, no la de Next). **Pero un `app/loading.tsx` global rompía un test que ya pasaba** (`property-detail.spec.ts` — "Una propiedad inexistente devuelve 404"): con un `loading.tsx` en la raíz, Next.js empieza a transmitir la respuesta con status 200 vía streaming/Suspense ANTES de que la página real llame a `notFound()`, así que el status code queda en 200 aunque el contenido mostrado sea correctamente el de "no encontrado" — un "soft 404" real, malo para SEO. **Se revirtió el `loading.tsx` global** en vez de dejar pasar esa regresión; agregar loading states por-ruta solo a páginas sin hijos dinámicos con `notFound()` (`/mercado`, `/favoritos`, `/vender`, etc.) queda como mejora opcional futura, no crítica hoy porque esas páginas ya cargan rápido vía ISR.
- **3.3 lifestyleContent — hallazgo que corrige el texto original de la auditoría:** el JSX para renderizarlo ya está agregado en `app/zonas/[zona]/page.tsx` (seguro, no rompe nada si está vacío), pero se confirmó contra la base real que **`lifestyleContent` es `null` para las 4 zonas** — la auditoría decía "el dato ya existe en la base", que no es correcto. Sumar contenido real de "estilo de vida" requeriría la misma investigación con fuentes citables que se hizo para transporte/colegios en una fase anterior, o pedírselo directamente a De Paola — no se inventó contenido para no violar la regla de honestidad del proyecto. Queda como pendiente de contenido, no de código (ver sección D).
- **3.4 BreadcrumbList:** agregado a las 4 páginas con breadcrumb visual (ficha de propiedad, emprendimiento, zona, nosotros) reusando el mismo array de items que ya arma el HTML — nuevo `BreadcrumbJsonLd` en `components/seo/json-ld.tsx`. Verificado con JSON-LD real parseado en el navegador.

Suite completa de Playwright en verde (30/30, incluye 5 tests nuevos de SEO — uno de ellos es explícitamente un test de regresión para el bug del `loading.tsx` que se encontró y revirtió). Lint y build limpios.
...
**FASE 4 — ✅ COMPLETA, con 2 eventos honestamente sin disparo y 3 bugs de accesibilidad reales encontrados vía Lighthouse (2026-09-02).**

- **4.1 GA4:** `@next/third-parties@16.3.3` (paquete oficial mantenido por Vercel, versión pineada a la misma versión exacta de Next.js del proyecto) en vez de instalar `gtag` a mano. `<GoogleAnalytics gaId={GA_MEASUREMENT_ID} />` en `app/layout.tsx`, condicionado a que `NEXT_PUBLIC_GA_MEASUREMENT_ID` esté seteada (nunca renderiza con un ID vacío). Se reutilizó el nombre de env var `NEXT_PUBLIC_GA_MEASUREMENT_ID` que ya existía documentado en `.env.example` de una fase anterior del proyecto, en vez del `NEXT_PUBLIC_GA_ID` que proponía este plan originalmente — mismo propósito, no se crea una segunda var redundante.
- **4.2 Los 12 eventos:** `lib/analytics.ts` con `trackEvent(name, params)` tipado (`AnalyticsEvent` como union type, no string suelto) y guarda `typeof window.gtag !== "function"`. **10 de los 12 disparan de verdad**, correlacionados con los flujos reales: `property_view` (nuevo `components/analytics/track-on-mount.tsx`, necesario porque la ficha es un Server Component y `window.gtag` no existe en el servidor), `property_search` (`search-results.tsx`, dispara con todos los params de query + `resultCount`), `favorite_property` (un solo punto en `action-panel.tsx` y en el grid de búsqueda, solo al guardar, nunca al sacar), `whatsapp_click`/`contact_click` (ficha de propiedad), `valuation_start`/`valuation_submit`/`visit_request` (wizard de tasación), `lead_created` (los 5 wizards de leads + el form de emprendimientos, capturando el `leadId` real que las server actions ya devolvían y antes se descartaba). **`share_property` y `ai_search` quedan sin disparo** — no hay botón de compartir ni buscador con IA en el producto todavía (`ai_search` depende de la Fase 8), así que dispararlos sería inventar telemetría de una feature que no existe. Quedan definidos en el tipo `AnalyticsEvent` para cuando esas features se construyan.
- **4.3 Search Console:** sigue bloqueado — necesita acceso real a la cuenta de Google de De Paola, no se puede simular ni inventar (ver sección D).
- **4.4 Lighthouse baseline:** corrido de verdad contra el deploy en vivo (`npx lighthouse` real, no estimado) en Home/`/propiedades`/ficha de propiedad. Números completos en `PERFORMANCE-BASELINE.md`. **Encontró 3 bugs de accesibilidad reales que no estaban en la auditoría original**, corregidos en esta misma fase:
  - `select-name`: 5 `<Select>` del buscador (`components/search/filter-panel.tsx`) solo tenían `placeholder`, sin nombre accesible para lectores de pantalla — se agregó `aria-label` a los 5.
  - `heading-order`: el Footer (`components/layout/footer.tsx`) saltaba de `h1`/`h2` de cada página directo a `h4` en sus columnas — se cambiaron ambos títulos de columna a `h3`.
  - `color-contrast`: dos casos, mismo origen — la dimensión visual `opacity-[0.55]` que marca las propiedades de ejemplo (`isSample`) en `components/ui/property-card.tsx` se aplicaba a TODA la tarjeta, incluyendo el badge Venta/Alquiler y el label de barrio, arrastrando su contraste por debajo de 4.5:1 (confirmado con la matemática exacta: `#57534a` al 55% de opacidad sobre `#faf8f3` da `#a09d96`, el valor exacto que midió Lighthouse). Las tarjetas reales (no-ejemplo) ya cumplían contraste sin cambios. Fix: la opacidad ahora se aplica solo a la imagen (`<Image>`), no al badge ni al texto — se preserva la señal visual de "esto es un ejemplo" en la foto sin arrastrar contenido legible por debajo del mínimo AA.
- **LCP fuera de objetivo en Home (4.7s) y ficha (5.0s)** — sospecha razonable documentada en `PERFORMANCE-BASELINE.md` (video del hero, galería de imágenes), no investigado a fondo en esta fase porque no estaba en el alcance de "Analytics" — candidato para una fase de performance dedicada si el usuario lo prioriza.

Suite completa de Playwright en verde (36/36, incluye 6 tests nuevos de analytics en `tests/analytics.spec.ts` — 2 de ellos necesitaron `page.waitForFunction` para esperar el evento async en vez de asumirlo disponible inmediatamente después de la navegación, mismo patrón de flakiness ya visto en fases anteriores). Lint, `tsc --noEmit` y build limpios.
...
**FASE 5 — ✅ COMPLETA, con una corrección real sobre el plan original y un bug de infraestructura de tests encontrado y corregido (2026-09-02).**

- **5.1 Filtro por característica:** checkboxes generados en runtime desde `PropertyFeature` (nunca hardcodeados), mismo markup que "Cochera", en `<fieldset>/<legend>`. Dos hallazgos reales al mirar los datos: `"jardín"`/`"jardin"` cargadas como keys distintas por tipeo inconsistente entre agentes — se agrupan por key normalizada (sin tocar los datos); `"garage"`/`"cochera"` se excluyen del listado porque duplican el checkbox "Cochera" que ya existía (ligado a `hasGarage`) — sumarlas hubiera puesto dos casillas "Cochera" en el mismo form.
- **5.2 Orden de resultados:** nuevo `<Select>` en la fila del contador. "Precio: menor/mayor" agrupa primero por moneda (USD antes que ARS, no alfabético) en vez de comparar los números crudo — el inventario real mezcla ARS y USD (un local en ARS 700.000 junto al resto en USD) y compararlos directo daría un orden sin sentido. Sin precio cargado queda al final, nunca se oculta. Bug real corregido en el camino: `search-results.tsx` (client component) importaba un valor en runtime desde `lib/search.ts`, que también trae la conexión de Prisma — rompía el build del bundle de cliente (`pg` no resuelve en el browser). Se movió la constante a `lib/property-options.ts`, que ya era client-safe.
- **5.3 Mapa real en `/zonas/[zona]` — corrección real sobre el plan original:** el plan decía "centrado en `neighborhood.lat/lng`", pero **`Neighborhood` no tiene lat/lng propio en el schema** — solo `PropertyLocation` lo tiene, por propiedad. Se implementó `getNeighborhoodCenter()` en `lib/search.ts`, que promedia las coordenadas de las propiedades activas de la zona; si ninguna tiene coordenadas, se mantiene el fallback "no disponible" que ya existía. El mapa nunca queda vacío: si un filtro deja 0 resultados visibles, se sigue mostrando el marcador central. Se extrajo `components/map/base-map.tsx` (tiles + ícono) desde `property-map.tsx`, compartido ahora por los 3 mapas del sitio.
- **5.4 Clustering (Decisión D3):** `leaflet.markercluster` integrado directo, sin sumar `react-leaflet-cluster` como segunda dependencia — manejado imperativamente con `useMap()` en `components/map/clustered-markers.tsx`. Ícono de clúster custom en el navy de la marca (`#00385C`) en vez del celeste default. Con el inventario real (11 propiedades, dispersas en Zona Norte) el clustering casi no se nota a zoom normal, tal como anticipaba el plan — el test lo fuerza zoomeando al mínimo, donde cualquier conjunto de puntos cae en el mismo radio de clúster, para no depender de coordenadas reales exactas y frágiles.
- **5.5 Newsletter segmentado:** modelo nuevo `NewsletterSubscriber` (aditivo — diff revisado antes de `db push`, solo `CREATE TYPE`/`CREATE TABLE`, cero riesgo). Tabla separada de `Lead` a propósito. Chips de segmento como `radiogroup` real (`role="radio"` + `aria-checked` + roving tabindex, navegable con flechas) en vez de un `<select>` genérico.
- **Bug real de infraestructura de tests encontrado y corregido:** con `fullyParallel: true` en `playwright.config.ts`, un `test.afterAll` corre **una vez por worker**, no una vez al final de toda la corrida. El `afterAll` original de `newsletter.spec.ts` borraba por prefijo de email — un worker que terminaba antes podía borrar la fila que OTRO worker acababa de precargar para el test de "email duplicado", haciendo que la segunda inserción pareciera nueva en vez de duplicada. Diagnosticado con un test aislado que confirmó que Prisma/Postgres detectaban el duplicado correctamente — el problema era la limpieza cruzada entre workers, no el código de producto. Se reemplazó por limpieza puntual del email exacto que cada test crea.

Suite completa de Playwright en verde (47/47), **confirmado estable en 3 corridas consecutivas completas** bajo la misma carga en paralelo que originalmente expuso el bug de limpieza cruzada. Lint, `tsc --noEmit` y build limpios.
...
**FASE 6 — PARCIAL, la parte que no depende de comprar el dominio (2026-09-02, a pedido explícito del usuario: "segui con la fase 6 lo que se pueda sin dominio").**

- **6.1 (parte de código lista, falta la parte de cuenta):** se auditaron `lib/site-url.ts`, `app/sitemap.ts`, `app/robots.ts`, `metadataBase` (`app/layout.tsx`) y el JSON-LD (`organization-schema.tsx`, `BreadcrumbJsonLd`) — los 5 ya leían `SITE_URL` desde la env var (`NEXT_PUBLIC_SITE_URL`), ninguno tenía el dominio hardcodeado. **No hubo nada que corregir acá — ya estaba bien.** El día que haya dominio, el cambio es una sola env var en Vercel, cero código. Hallazgo aparte (no arreglado, fuera de alcance de esta tarea): el logo (`SITE.logoUrl` en `lib/nav.ts`) apunta a un asset hosteado en el CDN de Wix (`static.wixstatic.com`) — si esa cuenta de Wix se da de baja, el logo se rompe en todo el sitio. Queda anotado para cuando se decida self-hostearlo.
- **6.2 Redirects — 3 confirmados en vivo, no una lista completa:** se verificó contra `www.depaolapropiedades.com` real (todavía en línea) cuáles de las URLs que la auditoría original observó siguen existiendo. `/campusnorte` y `/proyectoresidencial` devuelven 200 hoy; `/videocampusnorte` ya redirige a home desde el propio Wix. Hallazgo real: **`/proyectoresidencial` no es un proyecto residencial** — el `<title>` real de esa página es "REVISTA DIGITAL", así que mapea a `/insights`, no a `/emprendimientos` (el nombre del path hubiera sido una suposición incorrecta). Los 3 redirects (308, permanent) están en `next.config.ts`, no rompen nada hoy (esas rutas no existen en esta app) y ya están listos para el día que el dominio apunte para acá. **Esto NO es la lista completa** — el listado real de todo lo que Google tiene indexado del sitio viejo sigue necesitando Search Console (bloqueado, sección D).
- **6.3 Monitoreo de errores — corrección real sobre lo que ya existía:** `error.tsx`/`global-error.tsx` ya tenían un `console.error(error)`, pero como son Client Components ese log corre en el navegador del usuario — **nunca llegaba a los logs de Vercel**, contradiciendo lo que el plan original pedía como mínimo. Se agregó una ruta `app/api/log-error/route.ts` (server-side, sin dependencias nuevas) a la que ambos boundaries mandan el error por `fetch` best-effort; ahora sí queda en los Runtime Logs de Vercel. Sentry sigue sin agregarse — decisión explícita pendiente, no asumida.
- **6.4 Backups — documentado, no verificable desde acá:** nuevo `RUNBOOK.md` con el proceso real de restore de Neon (point-in-time recovery vía branching, no `pg_dump` tradicional) y un backup manual de respaldo con `pg_dump`/`pg_restore`. **No se pudo confirmar el plan/retención actual de Neon** — este agente solo tiene la cadena de conexión (`DATABASE_URL`), no acceso al dashboard de la cuenta. Queda explícito en el runbook como lo primero a confirmar antes de confiar en él en un incidente real.

Suite completa de Playwright en verde (51/51, incluye 4 tests nuevos: 2 de redirects, 2 de la ruta de logging de errores). Lint, `tsc --noEmit` y build limpios.

**Lo que sigue bloqueado en Fase 6, sin cambios:** 6.1 (comprar/configurar el dominio en sí) y la lista completa de 6.2 (Search Console del dominio viejo) — ambos son decisiones/accesos de negocio, no código.
