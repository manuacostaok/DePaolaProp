...
**Objetivo:** cerrar los dos 404 enlazados desde el sitio completo y diagnosticar (no adivinar) el error de hidratación.

| Tarea | Archivos | Cambios | Riesgos |
|---|---|---|---|
| 2.1 Diagnosticar hydration error #418 | — (solo diagnóstico, sin cambios de código todavía) | Correr `npm run dev` (build no minificado) y abrir Home + `/propiedades` con la consola de React DevTools activa, capturar el mensaje completo (no solo el código minificado). Candidatos a revisar en orden: `inert={transparent}` en `components/layout/header.tsx` (confirmado en código), luego `HeroVideo`, luego `WhatsAppFloat`. | Ninguno — es investigación pura antes de tocar nada. |
| 2.2 Corregir la causa encontrada en 2.1 | Depende del diagnóstico — más probable `components/layout/header.tsx` | Si es `inert`: mover el atributo a aplicarse solo post-hidratación vía `useEffect`+`ref` en vez de en el render inicial (evita que el server renderice un valor que el cliente recalcula distinto en el primer paint), o envolver la lógica de `transparent` para que su valor inicial coincida exactamente entre servidor y cliente. | Cambiar el mecanismo del header transparente-a-sólido es sensible visualmente — cualquier fix acá necesita verificación visual manual en el navegador real (no solo que desaparezca el error de consola), en desktop y mobile. |
