...
**Fases 1, 2, 3 y 4 completas, verificadas en producción real y pusheadas a `main`.** Cada fase corrió su propia ronda de `tsc`/lint/build/Playwright antes de commitear; la suite completa terminó en 36/36 tests verdes.
...
- **Fase 4 (Analytics):** GA4 (`@next/third-parties`) wireado y 10 de los 12 eventos de la Fase 20 instrumentados de verdad (`share_property`/`ai_search` deliberadamente sin disparo — no hay feature todavía). Lighthouse real corrido contra producción (`PERFORMANCE-BASELINE.md`), que encontró y permitió corregir 3 bugs de accesibilidad reales no listados en la auditoría original (`select-name`, `heading-order`, `color-contrast` — este último con causa raíz no obvia: la opacidad que dimeriza las tarjetas de ejemplo arrastraba el badge y el label de barrio por debajo del contraste mínimo, matemáticamente confirmado). Detalle completo en la sección de Fase 4 más abajo.

**Se avanzó por instrucción explícita del usuario ("segui con lo q haga falta") más allá del punto donde este agente se había detenido solo.** Quedan bloqueados por acceso/decisión de negocio, no por código: Fase 4.1 necesita un ID real de GA4 (`NEXT_PUBLIC_GA_MEASUREMENT_ID` no está seteada todavía, ni local ni confirmada en Vercel — el código ya tolera que falte) y Fase 4.3 necesita acceso a Google Search Console. Fase 5 agrega un modelo nuevo de Prisma (`NewsletterSubscriber`) y toca superficie de UX más amplia — es un buen punto para revisar antes de seguir, pero no bloquea: se continúa según la misma instrucción. El resto del documento de abajo es el plan original sin cambios.
