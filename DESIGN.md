...
- **Approach:** intentional — cada animación tiene un trigger claro (montaje una vez, scroll una vez, o loop continuo cuando es textura de fondo explícitamente pedida — ver excepción de `ImageMarquee` abajo).
...
- **Excepción explícita — texturas de fondo en Home (`ImageMarquee`, `components/ui/image-marquee.tsx`):** franja de imágenes reales en loop CSS infinito (`animate-marquee`, `app/globals.css`), muy tenue (opacity ~0.22 + grayscale + scrim del color de fondo de la sección encima), temática por capítulo — propiedades en 01, barrios en 02, el motivo de skyline (`ArchitectureMark`) a la deriva en 03, equipo en 04 si hay ≥3 fotos reales, tapas de artículo en 05 si hay ≥3. Se omite por completo si no hay suficiente material real (ver Do's/Don'ts: nunca repetir 1-2 fotos hasta que se vea roto). Pausa con `prefers-reduced-motion`. Solo Home — buscador/ficha siguen sin este patrón.
...
| 2026-09-10 | Home suma texturas de fondo animadas por capítulo (`ImageMarquee`) — loop CSS continuo, algo que la regla de Motion anterior ("nunca... en loop") no contemplaba | Pedido explícito del usuario: "en las secciones... algo animado que tenga que ver con la sección". Se acota a Home (buscador/ficha mantienen la regla dura sin motion decorativo) y a opacity muy baja + scrim para no competir con el contenido |
