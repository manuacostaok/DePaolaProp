...
**chapter-heading** (`components/ui/chapter-heading.tsx`, Etapa 2): device estructural de la Home — número de capítulo (01–06) + eyebrow + título, porque la Home se lee como una secuencia real (Propiedades → Zonas → Diferencial → Agentes → Editorial → Contacto, directo después del Hero — la pausa editorial sin numerar que vivía entre ambos se eliminó, ver Decisions Log 2026-09-10), no como numeración decorativa. No usar este patrón fuera de una secuencia genuina.
...
- **Excepción explícita — texturas de fondo en Home (`DriftBand`/`MotifRow`, `components/ui/section-motifs.tsx`):** franja de ilustraciones de línea (mismo lenguaje que `ArchitectureMark`: trazo fino, `currentColor`, "plano de arquitecto") en loop CSS infinito (`animate-marquee`, `app/globals.css`) — casas al cierre de 01, el skyline de `ArchitectureMark` al cierre de 02 y 03 (dirección/velocidad distinta en cada una para variar), personas al cierre de 04, documentos al cierre de 05. Reemplaza un primer intento con fotos reales en carrusel: el usuario pidió dibujos, no fotografías. **Importante — NO es un fondo `absolute` detrás del contenido:** un primer intento así (banda "absolute bottom" de la sección) terminaba, en mobile con todo apilado a una columna, cayendo literalmente encima de tarjetas/texto reales en vez de en margen vacío (la sección es mucho más alta ahí que en desktop). `DriftBand` es una franja de flujo normal con su propio alto reservado (`mt-16` de separación), después de todo el contenido de la sección — estructuralmente imposible que se superponga con nada, en cualquier viewport. Por eso puede ir a opacity ~0.32–0.5 (bastante marcado, pedido explícito del usuario) sin ningún riesgo de contraste. Pausa con `prefers-reduced-motion`. Solo Home — buscador/ficha siguen sin este patrón.
...
| 2026-09-10 | `DriftBand` pasa de "fondo `absolute bottom` de la sección" a franja de flujo normal después del contenido (`mt-16`, no overlap posible); opacity sube de ~0.14-0.16 a ~0.32-0.5; `preserveAspectRatio="xMidYMax slice"` en vez de `meet` para que los íconos llenen todo el alto disponible en vez de encogerse en contenedores angostos | El usuario reportó overlap real en mobile (la banda "absolute bottom" caía sobre tarjetas/texto cuando la sección se estiraba en una sola columna) y pidió más presencia visual, con énfasis en mobile por ser donde más se ve |
| 2026-09-10 | Hero: la bajada saca la enumeración de los 4 barrios ("Martínez, Florida, Vicente López y Villa Martelli") y la reemplaza por "Liderazgo en Zona Norte" | El usuario notó que nombrar solo esos 4 barrios sonaba a que la cobertura es solo ahí, cuando en realidad es toda la Zona Norte |
| 2026-09-10 | Íconos de línea de Home ganan variedad real por variante en vez de repetir el mismo dibujo: casas/departamentos cicla 3 siluetas (casa, edificio, casa moderna), personas alterna hombre/mujer, Zonas cambia el skyline reciclado por una escena de barrio propia (casa + cartel "SE VENDE" con la marca + perro + sol) | Pedido explícito del usuario: "no quiero que sea tan genérico... que sean distintas... que en los barrios hayan perros, sol, carteles de se vende de depaola" |
| 2026-09-10 | Se elimina la pausa editorial sin numerar entre el Hero y el capítulo 01 (era su propia sección de solo texto) — su mensaje pasa a ser la `description` del capítulo 01 (Propiedades) en vez de ocupar una pantalla propia | El usuario sintió que esa sección sobraba ("siento que está demás") y pidió cambiar el texto e integrarlo en una sección adecuada — el mensaje ("elegimos cada propiedad como quien va a vivir ahí") es sobre curaduría de propiedades, así que encaja mejor como bajada del capítulo de Propiedades que como quiebre propio |
| 2026-09-10 | `DriftBand` vuelve a ser `position: absolute` (con `z-index: -10` para pintar detrás del contenido estático sin importar el orden del DOM) en vez de franja de flujo normal (`mt-16`) — el alto por defecto baja a `h-16 sm:h-24` (64px/96px), siempre por debajo del padding vertical real de la sección (`py-20 sm:py-28` = 80px/112px), así la franja cae garantizado dentro del margen que la sección ya reserva vacío, nunca sobre contenido, sin agregar ni un px de alto a la página | El usuario corrigió el criterio anterior: "esos dibujitos que pusimos tenian que estar por atras y no ocupar espacio de la pagina" — la solución de la vuelta anterior (franja después del contenido) evitaba el overlap pero violaba el pedido original de que fuera puro fondo sin bulto propio |
| 2026-09-10 | "Nuestro equipo" suma 3 agentes de ejemplo (Carolina Ibáñez, Mariana Sosa, Diego Fernández — `isSample: true`, un especialista por barrio para Martínez/Florida/Vicente López) con fotos generadas por IA (StyleGAN2, no son personas reales) autohospedadas en `/public/agents`; `AgentCard` y la ficha de agente suman el mismo badge "Ejemplo" que ya usa `PropertyCard`, y ninguno tiene bio/teléfono/email/whatsapp cargado a propósito | Pedido explícito del usuario: "en los empleados rellena con personas hechas con ia para tener mejor esa parte" — mismo criterio de honestidad que el resto del sitio (nunca presentar contenido de ejemplo como real sin marcarlo) |
